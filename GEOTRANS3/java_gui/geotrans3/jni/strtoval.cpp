...
SVC_Status String_to_Projection(const char *str, MSP::CCS::CoordinateType::Enum* val)
{
  ...
  else if (strstr("LOCAL SPHERICAL", str))
  {
    *val = CoordinateType::localSpherical;
  }
  else if (strstr("GEOCENTRIC SPHERICAL", str))
  {
    *val = CoordinateType::spherical;
  }
...
SVC_Status Projection_to_String(const CoordinateType::Enum val, char str[32])
{
  ...
  case CoordinateType::localSpherical:
    {
      strcpy(str, "Local Spherical");
      break;
    }
  case CoordinateType::spherical:
    {
      strcpy(str, "Geocentric Spherical");
      break;
    }
  ...
  char  *String)
{
  ...
  case CoordinateType::localSpherical:
    {
      sprintf(String,"%s%s%s%s",in_out," Local Spherical Coordinates:",Separator,Separator);
      break;
    }
  case CoordinateType::spherical:
    {
      sprintf(String,"%s%s%s%s",in_out," Geocentric Spherical Coordinates:",Separator,Separator);
      break;
    }
  case CoordinateType::mercatorStandardParallel:
    {
