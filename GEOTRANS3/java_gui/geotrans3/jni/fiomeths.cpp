...
long Fiomeths::parseInputFileHeader(FILE *file)
{
  ...
  switch( sourceProjectionType )
  {
    ...
    case CoordinateType::localSpherical:
      if( localCartesianParameters )
        delete localCartesianParameters;
      localCartesianParameters = new LocalCartesianParameters(CoordinateType::localSpherical, originLongitude, originLatitude, originHeight, orientation);
      break;
    case CoordinateType::spherical:
      if( coordinateSystemParameters )
        delete coordinateSystemParameters;
      coordinateSystemParameters = new CoordinateSystemParameters(CoordinateType::spherical);
      break;
    case CoordinateType::mercatorStandardParallel:
    {
      if( mercatorStandardParallelParameters )
   ...
   CoordinateSystemParameters* _targetParameters )
{
  ...
  if ((targetProjectionType == CoordinateType::britishNationalGrid) ||
      ...
      (targetProjectionType == CoordinateType::localSpherical) ||
      (targetProjectionType == CoordinateType::spherical) ||
        ...
        if(!tempErrorCode)
        {
          ...
          if (!feof(inputFile) && (fscanf(inputFile, "%255[^,\t\n#;]", buf)))
          {
            ...
            if(!tempErrorCode)
            {
                ...
                SphericalCoordinates* coordinate = NULL;
                switch(sourceProjectionType)
                {
                    case CoordinateType::localSpherical:
                        coordinate = new SphericalCoordinates(CoordinateType::localSpherical);
                        coordinate->setLocalSphericalCoord(longitude, latitude, height);
                        break;
                    case CoordinateType::spherical:
                        coordinate = new SphericalCoordinates(CoordinateType::spherical);
                        coordinate->setSphericalCoord(longitude, latitude, height);
                        break;
                }
                return coordinate;
            }
...
CoordinateTuple* Fiomeths::initTargetCoordinate()
{
  switch( targetProjectionType )
  {
    ...
    case CoordinateType::localSpherical:
      return new SphericalCoordinates(CoordinateType::localSpherical);
    case CoordinateType::spherical:
      return new SphericalCoordinates(CoordinateType::spherical);
    case CoordinateType::mercatorStandardParallel:
...
void Fiomeths::writeTargetCoordinate( CoordinateTuple* targetCoordinate )
{
  ...
  switch( targetProjectionType )
  {
    ...
    case CoordinateType::localSpherical: 
    {
      char latitude_str[17];
      char longitude_str[17];
      if( dynamic_cast< SphericalCoordinates* >( targetCoordinate ) )
      {
        SphericalCoordinates coord = ( *dynamic_cast< SphericalCoordinates* >( targetCoordinate ) );
        if( outputLatitudeLongitudeOrder )  // first value is elevationAngle, second value is azimuth
        {
          Latitude_to_String( ( coord.elevationAngle()*180 )/PI, latitude_str, _useNSEW, _useMinutes, _useSeconds );
          fprintf(outputFile, "%s", latitude_str);
          fprintf(outputFile, ", ");
          Longitude_to_String( ( coord.azimuth()*180 )/PI, longitude_str, _useNSEW, _useMinutes, _useSeconds );
          fprintf(outputFile, "%s", longitude_str);
        }
        else
        {
        ...
        fprintf(outputFile, ", ");
        Meter_to_String(coord.range(), meter_str);
        fprintf(outputFile, "%s", meter_str);
      }
      else
      {
    ...
    case CoordinateType::spherical:
    {
      ...
      if( dynamic_cast< SphericalCoordinates* >( targetCoordinate ) )
      {
        ...
        if( outputLatitudeLongitudeOrder )  // first value is elevationAngle, second value is azimuth
        {
          Latitude_to_String( ( coord.sphericalLatitude()*180 )/PI, latitude_str, _useNSEW, _useMinutes, _useSeconds );
          fprintf(outputFile, "%s", latitude_str);
          fprintf(outputFile, ", ");
          Longitude_to_String( ( coord.sphericalLongitude()*180 )/PI, longitude_str, _useNSEW, _useMinutes, _useSeconds );
          fprintf(outputFile, "%s", longitude_str);
        }
        else
        {
        ...
        fprintf(outputFile, ", ");
        Meter_to_String(coord.radius(), meter_str);
        fprintf(outputFile, "%s", meter_str);
      }
      else
      {
