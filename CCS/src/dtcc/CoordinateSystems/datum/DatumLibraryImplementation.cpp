 ...
 *    10/02/17          M. Thakkar : LSC-13195 : Fixed array out of bounds error (DATUM_CODE_LENGTH)
 */
 ...
 */
  ...
  { /* Just copy */
    return new GeodeticCoordinates(
       CoordinateType::geodetic, sourceLongitude, sourceLatitude, sourceHeight);
  }
  else
  {
    if( _ellipsoidLibraryImplementation )
  ...
  index ++;
    ...
    while ( !feof(fp_7param ) )
    {
        ...
        else
        {
          ...
          {
            userDefined = true;
            for( int i = 0; i < DATUM_CODE_LENGTH - 1; i++ )
              code[i] = code[i+1];

            code[DATUM_CODE_LENGTH-1] = '\0';
          }
        }
 ...
 */
  ...
  CartesianCoordinates cart( CoordinateType::geocentric, X_WGS84, Y_WGS84, Z_WGS84 );
  GeodeticCoordinates* wgs84GeodeticCoordinates = geocentric84.convertToGeodetic( &cart );
 ...
 */
  ...
  Geocentric geocentric72( a_72, f_72 );
  CartesianCoordinates cart( CoordinateType::geocentric, X, Y, Z );
  GeodeticCoordinates* wgs72GeodeticCoordinates = geocentric72.convertToGeodetic( &cart );
