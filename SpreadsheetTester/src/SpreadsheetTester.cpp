...
///  coordinateSystemState[direction].coordinateSystem = 0;
  ...
  coordinateSystemState[direction].coordinates.sphericalCoordinates = 0;
 ...
 */
  ...
  switch( coordinateSystemState[direction].coordinateType )
  {
	...
	case CoordinateType::spherical:
      ...
      if( coordinateSystemState[direction].coordinates.sphericalCoordinates ) 
      {
        delete coordinateSystemState[direction].coordinates.sphericalCoordinates;
		coordinateSystemState[direction].coordinates.sphericalCoordinates = 0;
	  }
	  break;
    ...
    case CoordinateType::localCartesian:
      ...
      break;
	case CoordinateType::localSpherical:
	  if( coordinateSystemState[direction].parameters.localCartesianParameters ) 
      {
        delete coordinateSystemState[direction].parameters.localCartesianParameters;
        coordinateSystemState[direction].parameters.localCartesianParameters = 0;
    ...
    if (Set_Projection(SourceOrTarget::target, temp_output_projection)) 
    {
      ...
      switch (input_projection_type)
      {
        ...
        case CoordinateType::spherical:
        {
          ...
          coordinateSystemState[SourceOrTarget::source].coordinates.sphericalCoordinates =
             new MSP::CCS::SphericalCoordinates( input_projection_type );
          coordinateSystemState[SourceOrTarget::source].coordinates.sphericalCoordinates->setSphericalCoord( longitude, latitude, input_coord3 );
		  break;
        }
        case CoordinateType::localSpherical:
        {
          double az = input_coord2 * PI_OVER_180;
          double el = input_coord1 * PI_OVER_180;
          
          coordinateSystemState[SourceOrTarget::source].coordinates.sphericalCoordinates = new MSP::CCS::SphericalCoordinates( input_projection_type );
          coordinateSystemState[SourceOrTarget::source].coordinates.sphericalCoordinates->setLocalSphericalCoord( az, el, input_coord3 );
		    break;
		}
        case CoordinateType::geodetic:
        {
...
void SpreadsheetTester::convertSourceToTarget()
{
  ...
  switch( coordinateSystemState[SourceOrTarget::source].coordinateType )
  {
    ...
    case CoordinateType::localSpherical:
    {
      _sourceCoordinates = coordinateSystemState[SourceOrTarget::source].coordinates.sphericalCoordinates;
      break;
    }	  
    case CoordinateType::geodetic:
    {
  ...
  switch( coordinateSystemState[SourceOrTarget::target].coordinateType )
  {
    ...
    case CoordinateType::geocentric:
    {
      MSP::CCS::CartesianCoordinates* _targetCoordinates = 
	     new MSP::CCS::CartesianCoordinates(CoordinateType::geocentric);
      try
      {
        coordinateConversionService->convertSourceToTarget( _sourceCoordinates, &sourceAccuracy, *_targetCoordinates, targetAccuracy );
   ...
   case CoordinateType::spherical:
	{
      MSP::CCS::SphericalCoordinates* _targetCoordinates =
         new MSP::CCS::SphericalCoordinates(CoordinateType::spherical);
      try
      {
         coordinateConversionService->convertSourceToTarget(
         _sourceCoordinates, &sourceAccuracy, *_targetCoordinates, targetAccuracy );
         coordinateSystemState[SourceOrTarget::target].coordinates.sphericalCoordinates = _targetCoordinates;
      }
      catch(CoordinateConversionException e)
      {
         _targetCoordinates->setErrorMessage(e.getMessage());
    ...
    case CoordinateType::localSpherical:
	 {
	  MSP::CCS::SphericalCoordinates* _targetCoordinates =
         new MSP::CCS::SphericalCoordinates(CoordinateType::localSpherical);
      try
	  {
	    coordinateConversionService->convertSourceToTarget(
		 _sourceCoordinates, &sourceAccuracy, *_targetCoordinates, targetAccuracy );
  ...
  if (!file_read_error)
  {
    ...
    switch (output_projection_type)
    {
      ...
      case CoordinateType::localCartesian:
      {
	   ...
	   case CoordinateType::localSpherical:
      {
        if( coordinateSystemState[SourceOrTarget::target].coordinates.sphericalCoordinates )
        {
          MSP::CCS::SphericalCoordinates sphericalCoordinates =
             *coordinateSystemState[SourceOrTarget::target].coordinates.sphericalCoordinates;
          if (strlen( sphericalCoordinates.errorMessage() ) > 0 )
            fprintf(output_file, "n/a\tn/a\tn/a\t\t");
          else
          {
            double azimuth    = sphericalCoordinates.azimuth();
            double elevation  = sphericalCoordinates.elevationAngle();
            double range      = sphericalCoordinates.range();

            fprintf(output_file, "%.6f\t", elevation * _180_OVER_PI);
            ...
            if (output_height)
            {
              fprintf(output_file, "%.6f\t%.6f\t\t", azimuth * _180_OVER_PI, range);
              // azimuth test is used to check for -180, 180 cases
              if ((fabs(elevation * _180_OVER_PI - output_coord1) > .172) ||
                 ((fabs(azimuth * _180_OVER_PI - output_coord2) > .172) &&
                 (fabs(azimuth * _180_OVER_PI) - fabs(output_coord2)) > .1) || (fabs(range - output_coord3) > 2.0))
              {
                if (strlen( sphericalCoordinates.warningMessage() ) > 0 )
                {
                  warning_different = TRUE;
                  warning_different_count++;
                  printf("Test Case Id: %s - Expected: %.6f, %.6f, %.6f   Calculated: %.6f, %.6f, %.6f - Warning: %s\n", id, output_coord1, output_coord2, output_coord3, elevation * _180_OVER_PI, azimuth * _180_OVER_PI, range, sphericalCoordinates.warningMessage()); 
                }
                else
                {
                  successful_different = TRUE;
                  successful_different_count++;
                  printf("Test Case Id: %s - Expected: %.6f, %.6f, %.6f   Calculated: %.6f, %.6f, %.6f \n", id, output_coord1, output_coord2, output_coord3, elevation * _180_OVER_PI, azimuth * _180_OVER_PI, range); 
                }
              }
              else
              {
                ...
                { 
                  ...
                  warning_equivalent_count++; 
                  if( showWarnings )
                    printf("Test Case Id: %s - Warning: %s\n", id, sphericalCoordinates.warningMessage()); 
                }
                else
                {
                  successful_equivalent = TRUE;
      ...
      case CoordinateType::spherical:
      {
        if( coordinateSystemState[SourceOrTarget::target].coordinates.sphericalCoordinates )
        {
          ...
          {
            double longitude = sphericalCoordinates.sphericalLongitude();
            double latitude  = sphericalCoordinates.sphericalLatitude();
            ...
            if (output_height)
            {
              fprintf(output_file, "%.6f\t%.6f\t\t", longitude * _180_OVER_PI, radius);
              // longitude test is used to check for -180, 180 cases
              if ((fabs(latitude * _180_OVER_PI - output_coord1) > .172) || ((fabs(longitude * _180_OVER_PI - output_coord2) > .172) && (fabs(longitude * _180_OVER_PI) - fabs(output_coord2)) > .1) || (fabs(radius - output_coord3) > 2.0))
              {
                if (strlen( sphericalCoordinates.warningMessage() ) > 0 )
                {
                  warning_different = TRUE;
                  warning_different_count++;
                  printf("Test Case Id: %s - Expected: %.6f, %.6f, %.6f   Calculated: %.6f, %.6f, %.6f - Warning: %s\n", id, output_coord1, output_coord2, output_coord3, latitude * _180_OVER_PI, longitude * _180_OVER_PI, radius, sphericalCoordinates.warningMessage()); 
                }
                else
                {
                  successful_different = TRUE;
                  successful_different_count++;
                  printf("Test Case Id: %s - Expected: %.6f, %.6f, %.6f   Calculated: %.6f, %.6f, %.6f \n", id, output_coord1, output_coord2, output_coord3, latitude * _180_OVER_PI, longitude * _180_OVER_PI, radius); 
                }
              }
              else
              {
